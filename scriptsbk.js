document.addEventListener('DOMContentLoaded', () => {
    // -----------------------------------
    // SECTION 1: Landing Page (index.html) - Girly Hippy Entry
    // -----------------------------------
    ...
    // -----------------------------------
    // SECTION 2: Puzzle Page (puzzle.html) - Tango Logic Puzzle
    // -----------------------------------
    if (window.location.pathname.includes('puzzle.html') || document.querySelector('body.puzzle')) {
        ...
        const taunts = ['Step it up, Evee!', 'RUSUF grooves better!', 'Dance, Babbyy!'];
        const cells = [];
        const clues = [
        ...
        checkButton.addEventListener('click', () => {
            ...
            // Rule 1: 3 X and 3 Y per row/column
            let valid = true;
            for (let i = 0; i < 6; i++) {
            ...
            // Rule 2: No more than 2 adjacent
            for (let y = 0; y < 6; y++) {
                for (let x = 0; x < 4; x++) {
    ...
    // -----------------------------------
    // SECTION 3: Trippy Page (trippy.html) - Words Falling in All Directions
    // -----------------------------------
    if (window.location.pathname.includes('trippy.html') || document.querySelector('body.trippy')) {
        console.log('Trippy page loaded!');

        const poemText = "Love bends time, circuits hum Flesh and code, we succumb";
        ...
        const timerInterval = setInterval(() => {
            ...
            speedFactor = 1 + (5 * (30 - timeLeft) / 30);
        ...
        toPoem.addEventListener('click', () => {
            ...
            alert('Rusuf says: Noma sana, Evee!');
            window.location.href = 'poem.html';
        });
    }

    // -----------------------------------
    // SECTION 4: Poem Page (poem.html) - Display and Replay
    // -----------------------------------
    if (window.location.pathname.includes('poem.html') || document.querySelector('body.poem')) {
        console.log('Poem page loaded!');

        const replayButton = document.getElementById('replay');

        replayButton.addEventListener('click', () => {
            console.log('Replay clicked!');
            alert('Rusuf says: Rudia tena, Babbyy!');
...
also the poem words should be falling in the trippy page,the one with weed button,in all directions,from the middle ,left,in a trippy manner,others big,etc you get it